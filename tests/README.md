...
the NoCode policy or merging authentication updates. These in-memory checks cover
the routing decision, the documented tool allowlist, both supported C8Oforms token
audiences and Studio calls without the NoCode profile header. NoCode must reject
`log-view` and low-code tools without weakening the bearer authentication gate.
The tests do not launch agents, write application data, or replace live token/ACL
...
- `scripts/validate_setup_vibe_config.js`: Validates Vibe 2.25 MCP auth/header migration and deterministic `config.toml` patching without a running Studio.
