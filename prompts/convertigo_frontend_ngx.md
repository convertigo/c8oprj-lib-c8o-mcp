...
11. Open `mobile-builder-open(wait=false)` only after the first visible shell mutation exists, then continue work while it starts. Before browser smoke or final proof, poll with `mobile-builder-open(stateOnly=true, wait=true)` or a normal waited call. If a JxBrowser debug endpoint is returned, attach Playwright or browser-control MCP to that endpoint and test the feature in the visible Studio viewer.
