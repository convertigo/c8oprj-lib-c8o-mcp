...
4. Inspect only the exact target project or subtree required to answer the next bootstrap question.
...
- When the task is a standard CRUD path, capture the deterministic `spec` inputs early: project, driver family, connector name, facade prefix, entities, seed choice, and visible entry page.
- When the task is a standard CRUD path, prefer `convertigo-crud-fastpath` and the direct order documented in `convertigo-crud-fastpath`: `upsert-crud` -> `crud-proof` -> `upsert-ngx-crud-kit` -> final `crud-proof`.
