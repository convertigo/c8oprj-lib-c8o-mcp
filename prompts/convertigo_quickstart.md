...
- When the task is a standard CRUD path, prefer the direct order documented in `convertigo-crud-practical-cases`: `upsert-crud` -> `crud-status` -> `requestable-execute` proofs -> `upsert-ngx-crud-kit` -> final `crud-status`.
