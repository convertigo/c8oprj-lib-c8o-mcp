...
- For frontend work, call `mobile-builder-open(wait=false)` as soon as the UI project is known, continue inspection or mutation while it starts, then poll with `mobile-builder-open(stateOnly=true, wait=true)` before browser smoke or final proof. Prefer `viewerHomeUrl` or `viewerBaseUrl`. When a JxBrowser debug endpoint is returned, use Playwright or browser-control MCP against that endpoint to verify the visible Studio viewer. Reserve `DisplayObjects/mobile/home` for production builds.
