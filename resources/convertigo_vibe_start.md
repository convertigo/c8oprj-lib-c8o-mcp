...
- If slash placement between connector `baseDir` and transaction `subDir` is ambiguous, use one coherent single-slash shape and verify immediately with `requestable-execute`; do not alternate blindly between guessed paths. A root-level endpoint should usually use an empty `baseDir` and a `subDir` without a leading slash. A shared path prefix should usually live in `baseDir` without trailing slash, with `subDir` as the remaining path without leading slash.
- When a direct HTTP proof returns HTML, documentation, login content, or an error page for an expected JSON/XML API, do one path/variable/header repair and rerun proof. If the second proof has the same payload class or status problem, stop toggling the same provider settings; either choose another endpoint that satisfies the same requested contract or keep the HTTP rail and report the proof incomplete.
...
- Derive the `StoreResults` item path from the proven shaped facade payload, not from a memorized fallback. Accept final UI mappings from explicit facade fields such as `out.items`, `out.contract?.items`, `out.response?.items`, or another proven non-transport application object. Do not use `out.transaction`, `out?.transaction`, `response?.transaction`, `transaction.document`, `HttpInfo`, headers, or other raw transport subtrees in final `StoreResults`. If those paths seem necessary to reach records, the facade contract is incomplete: stop UI work, repair the facade so it emits `items`/`total`/`query` or equivalent application fields, rerun facade proof, then resume UI. If row text uses nested fields such as `record.properties.label`, `items` must be the shaped facade array whose elements actually contain `properties`.
...
- Do not change that copy XPath to `./array` just because `requestable-execute` displays the transaction response as a root JSON `array`; that display is the serialized response body, not the internal step XPath used by `sourceDefinition`. If the `./document/array` copy is empty, first verify that the transaction is still producing JSON records and that the `TransactionStep` priority is the real producer priority. If the copy returns a broad/unfiltered dataset, fix variable pass-through between the facade variable, `StepVariable`, and HTTP transaction variable; do not repair it by changing the copy XPath.
