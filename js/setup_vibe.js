...
(function () {
  ...
  function buildSkillMarkdown(mcpUrl) {
    ...
    return [
      ...
      "7. Read `convertigo://resources/convertigo-vibe-start`.",
      "8. Pick the smallest matching shared recipe before mutating anything.",
      "9. Do not call `Convertigo_rag-query` before the start guide and the chosen recipe were read.",
      "",
      "## MCP call discipline",
      "",
      ...
      "- Open the mobile builder early with `Convertigo_mobile-builder-open wait=false`, continue other work while it starts, then call `Convertigo_mobile-builder-open stateOnly=true wait=true` before live proof. If a JxBrowser debug endpoint is returned, attach Playwright or browser-control MCP to that endpoint for the visible Studio viewer proof.",
      ...
      "- For live mobile proof, prefer the URL returned by the waited `Convertigo_mobile-builder-open`. If it returns `browserDebugUrl`, `browserDevToolsJsonUrl`, or `browserDevToolsWebSocketUrl`, those values target the visible Studio mobile viewer; use them for Playwright or browser-control MCP smoke proof instead of opening an unrelated browser page.",
