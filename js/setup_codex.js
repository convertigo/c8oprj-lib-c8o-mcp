...
(function () {
  ...
  function buildSkillMarkdown(mcpUrl) {
    ...
    return [
      ...
      "- Keep structural proof compact: normally use `databaseobject-tree-get({target:\"<changed-root>\",childrenDepth:2,properties:\"changed\"})`. Do not combine a deep `properties:\"all\"` tree read with a waited builder call in one tool result; it inflates every later model turn without improving readiness evidence.",
      ...
      "- For timers, live updates, interactions, or other changing behavior, require two observations showing the expected change or an equivalent browser-control assertion. If browser control is unavailable, report the result as implemented but functionally unvalidated; do not claim success merely because compilation and server logs are green.",
