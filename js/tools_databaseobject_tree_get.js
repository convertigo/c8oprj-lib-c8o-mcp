...
function addIncludedNode(meta, ancestors, dbo, childCount) {
  var previous = null;
  ...
  currentNode.depth = meta.depth;
  currentNode.childrenCount = childCount;
  currentNode.hasChildren = childCount > 0;
...
function visitNode(dbo, depth, ancestors) {
  ...
  if (currentIndex >= startOffset) {
    if (returnedNodes < maxNodesValue) {
      addIncludedNode(meta, ancestors, dbo, childCount);
      returnedNodes += 1;
    } else {
      hasMore = true;
      stopTraversal = true;
