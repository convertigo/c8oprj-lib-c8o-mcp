...
| `palette-authoring-catalog`   | `tools_palette_authoring_catalog.yaml`     | Return the frontend palette options for one NGX parent as a paginable authoring catalog, with HTML/SCSS/text snippets for each local entry and optional marketplace library suggestions. |
...
| `palette-html-skeleton`       | `tools_palette_html_skeleton.yaml`          | Resolve one frontend palette entry against a parent QName and return an authoring surface (`html`, `scss`, or `text`) plus semantic marker metadata, so agents can compose `_private/ionic` snippets without mining other projects. |
| `palette-json-skeleton`       | `tools_palette_json_skeleton.yaml`          | Resolve one palette entry against a parent QName and return a minimal canonical Convertigo JSON subtree skeleton plus `coverage=template|serialized|hints`. Legacy helper; prefer `_private/ionic` HTML authoring for frontend work. |
| `palette-resolve-with-marketplace` | `tools_palette_resolve_with_marketplace.yaml` | Check the target palette first, then query/import marketplace libraries when needed, reread the palette, and return the refreshed result plus JSON skeleton and authoring HTML/SCSS/text when an exact class token is provided. Also supports capability-first discovery when only `search` / `filter` intent hints are known (for example charts, maps, barcode, editor, PDF viewer, calendar). |
...
| `project-reload`              | `tools_project_reload.yaml`                 | Reload a project from disk, discarding unsaved changes in memory. Use `fromIonic=true` to reimport supported `_private/ionic` frontend edits before reload, including `sharedComponent`, `sharedAction`, `invokeSharedAction`, and `useSharedComponent` sidecars. Add `ionicTarget` to import one generated Ionic file/directory into the loaded tree without a full project reload. Do not use `preserveIonic` for agent workflows; preserve edited authoring files explicitly when a backend reload could regenerate Ionic. |
...
| `palette-authoring-catalog` | `tools_palette_authoring_catalog` | Return a frontend authoring catalog for one NGX parent | Lists the local frontend palette entries allowed under the given NGX parent and returns an authoring surface (`html`, `scss`, or `text`) for each one; pass `includeMarketplace=true` to append marketplace library suggestions when capability hints are provided. |
...
| `palette-html-skeleton` | `tools_palette_html_skeleton` | Return an authoring HTML/SCSS/text skeleton for a frontend palette entry | Resolves a frontend palette class against a parent QName, reuses the canonical palette metadata, instantiates the live palette entry, and returns an authoring surface suitable for `_private/ionic` composition and round-trip import. |
...
| `project-reload` | `tools_project_reload` | Reload a project from disk | Reloads one project and discards unsaved in-memory changes. Use `fromIonic=true` to reimport supported `_private/ionic` frontend edits before the reload, including `sharedComponent`, `sharedAction`, `invokeSharedAction`, and `useSharedComponent` sidecars. Add `ionicTarget` to import one generated Ionic file/directory into the loaded tree without a full project reload. Do not use `preserveIonic` for agent workflows; move/restore edited authoring files around backend reloads instead. |
